...
这是一个静态网页，展示上海二手住宅价格指数换算后的估算房价 K 线，并叠加 EMA10、EMA21、SMA50、ATR、ADR 等指标。
...
- `shanghai-house-price-data.js`：由脚本生成的上海二手住宅指数数据。
...
也可以在 GitHub 仓库的 **Actions** 页面手动运行 `Update Shanghai House Price Chart`。
...
5. 到 **Actions** 页面手动运行一次 `Update Shanghai House Price Chart`。
